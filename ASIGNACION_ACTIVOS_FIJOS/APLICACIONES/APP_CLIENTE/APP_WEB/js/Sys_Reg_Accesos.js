...
// Variable Global
var xUbica = "", xElem = "", yMetodos = [], xSesion_Prueba = "", yUser = "", xBand_ACCESOS = 0,  xID_ACCESOS = 0, xCODIGO_ACCESOS = '', xID_ROL = 0;
    ...
    xMetodos.then(function(Valor){
        ...
        try {
            ...
            var xMi_Modelo_01 = function () {
                ...
                let lstObs = ['xReg_Total', 'g_current_page', 'search', 'g_nPagina', 'g_nextPage', 'g_nPorPagina', 'g_firstPage', 'g_previousPage', 'ID_Estado', 'ID_Usuario', 'ID_Paginas'];
                let lstObs_Arrays = ['Mis_Datos', 'g_lstTabla', 'lst_Estados', 'lst_Usuarios', 'lst_Paginas'];

                yMetodos[0](self, lstObs); yMetodos[0](self, lstObs_Arrays, true);

                // Botones del Encabezado
                self.Asignar_Enc = function () {
                    ...
                    // Se ejecuta la orden...
                    let Aplicar_Registros = async (lUSER, lROL, lSESION) => {
                        let Mi_url = "Frm_Metodos_WEB_Services.aspx/ASIGNAR_SYS_ACCESOS";
                        let Mis_Parametros = JSON.stringify({ xUSER: lUSER, xROL: lROL, xSESION: lSESION  });

                        let response = await yMetodos[4]('¿Desea GENERAR LOS ACCESOS al Registro actual?...', Mi_url, Mis_Parametros);
                        if (response.d.exito == false){
                            ...
                            self.ID_Estado(1); self.Datos_Registros(self.ID_Estado(), lUSER, lROL);
                        } 
                    };
                    Aplicar_Registros(self.ID_Usuario(), xID_ROL, ySESION_USER);
                };
                ...
                self.ID_Estado(1); 
                self.Datos_Registros = async ( yESTADO, yUSER, yROL ) => {
                    // Mis Registros
                    if ((yESTADO == null) || (yESTADO == undefined) || (yUSER == null) || (yUSER == undefined) || (yROL == null) || (yROL == undefined)) {
                        self.Mis_Datos([]); self.xReg_Total(true); 
                    } else {
                        $(".loader").fadeIn("slow");
                        let Mi_url = 'Frm_Metodos_WEB_Services.aspx/Listar_Accesos';
                        let Mis_Parametros = JSON.stringify({ xId_Estado: yESTADO, xUser: yUSER, xRol: yROL });
                ...
                self.Datos_Registros(self.ID_Estado(), self.ID_Usuario(), xID_ROL);

                self.load_Datos_ACCESOS = () => {
                    if ((self.ID_Estado() != null) || (self.ID_Estado() != undefined)){
                        if ((self.ID_Usuario() != null) || (self.ID_Usuario() != undefined)){
                            xID_ROL = parseInt(self.lst_Usuarios().find(x => x.ID == self.ID_Usuario()).DESCRIPCION.split('|')[1]);
                            self.Datos_Registros(self.ID_Estado(), self.ID_Usuario(), xID_ROL);
                        } else { self.Mis_Datos([]); self.xReg_Total(true); xID_ROL = 0; }
                    } else { self.Mis_Datos([]); self.xReg_Total(true); xID_ROL = 0; }
                };
                ...
                self.selectRow_Editar = function (item) {
                    ...
                    xID_ACCESOS = parseFloat(item.ID); xCODIGO_ACCESOS = item.CODIGO_01 + ' - ' + item.DESC;

                    $("#Chk_Ejecutar").prop('checked', item.EJEC);
                    $("#Chk_Visualizar").prop('checked', item.VIS);
                    $("#Chk_Imprimir").prop('checked', item.IMP);
                    $("#Chk_Borrar").prop('checked', item.DEL);
                    $("#Chk_Insertar").prop('checked', item.INS);
                    $("#Chk_Actualizar").prop('checked', item.ACT);
                ...
                self.selectRow_Reactivar = function(item){
                    ...
                    let Reactivar_Registros = async (lID, lUSER, lSESION) => {
                        let Mi_url = "Frm_Metodos_WEB_Services.aspx/Reactivar_SYS_ACCESOS";
                        ...
                        if (response.d.exito == false){
                            ...
                            xContenido = 'Acción Realizada con Exito...'; yMetodos[1](xContenido, "success"); 
                            self.ID_Estado(1); self.Datos_Registros(self.ID_Estado(), lUSER, xID_ROL);
                        } 
                    };
            ...
            // Ventanas Modales
            $("#Frm_Datos").dialog({
                autoOpen: false, width: (lastWidth * 0.80), height: 'auto', modal: true, resizable: true, show: "slide", hide: "slide",
                dialogClass: "Estilos_Ventanas_Modales",
                position: "center",
                closeOnEscape: false,
                open: function () {
                    $(this).dialog('option', 'title', 'EDITAR REGISTRO EXISTENTE: ' + xCODIGO_ACCESOS);
                ...
                buttons: [
                {
                    ...
                    click: function () {
                        ...
                        var xEJE = ""; var checkeado = $("#Chk_Ejecutar").is(":checked");
                        if (checkeado == true) {
                            xEJE = "1";
                        }
                        ...
                        var xVIS = ""; checkeado = $("#Chk_Visualizar").is(":checked");
                        if (checkeado == true) {
                            xVIS = "1";
                        }
                        else if (checkeado == false) {
                        ...
                        var xIMP = ""; checkeado = $("#Chk_Imprimir").is(":checked");
                        if (checkeado == true) {
                            xIMP = "1";
                        }
                        else if (checkeado == false) {
                        ...
                        var xBOR = ""; checkeado = $("#Chk_Borrar").is(":checked");
                        if (checkeado == true) {
                            xBOR = "1";
                        }
                        else if (checkeado == false) {
                        ...
                        var xINS = ""; checkeado = $("#Chk_Insertar").is(":checked");
                        if (checkeado == true) {
                            xINS = "1";
                        }
                        else if (checkeado == false) {
                        ...
                        var xACT = ""; checkeado = $("#Chk_Actualizar").is(":checked");
                        if (checkeado == true) {
                            xACT = "1";
                        }
                        else if (checkeado == false) {
                        ...
                        var xUSER = parseInt(md_Modelo_01.ID_Usuario()), xSESION = ySESION_USER;
                                                
                        var uEncabezado = [];

                        uEncabezado.push({ ID: xID_ACCESOS, EJEC: xEJE, VIS: xVIS, IMP: xIMP, DEL: xBOR, INS: xINS, ACT: xACT, USER: xUSER });

                        // Ejecutando la Orden
                        let Guardar_Registros = async (yDatos, lSESION) => {
                            let Mi_url = "Frm_Metodos_WEB_Services.aspx/ACTUALIZAR_ACCESOS";
                            ...
                            if (response.d.exito == false){
                                ...
                                xContenido = 'Acción Realizada con Exito...'; yMetodos[1](xContenido, "success"); 
                                xID_ACCESOS = parseFloat(response.d.Mensaje);
                            } 
                        };
                ...
                },
                {
                    ...
                    click: function () {
                        ...
                        md_Modelo_01.Datos_Registros(md_Modelo_01.ID_Estado(), md_Modelo_01.ID_Usuario(), xID_ROL);
                    },
                    class: "ui-button-spl2"
                }]
            }).dialog({
            ...
            $("#Frm_Baja").dialog({
                ...
                closeOnEscape: false,
                open: function () {
                    $(this).dialog('option', 'title', 'BAJA AL REGISTRO Nº - ' + xCODIGO_ACCESOS);
                ...
                buttons: [
                {
                    text: "GUARDAR",
                    click: function () {
                        var xOBS = ($.trim($("#Txt_Observacion_Baja").val()).length == 0) ? "BAJA AL REGISTRO Nº : " + xCODIGO_OPCIONES : $.trim($("#Txt_Observacion_Baja").val()).toUpperCase();
                        ...
                        let Baja_Registros = async (lID, lFECHA, lOBS, lUSER, lSESION) => {
                            let Mi_url = "Frm_Metodos_WEB_Services.aspx/Baja_SYS_ACCESOS";
                            ...
                            if (response.d.exito == false){
                                ...
                                // Limpiando Campos...
                                xID_ACCESOS = 0; xCODIGO_ACCESOS = '';

                                $("#Txt_Fecha_Baja").val(date_str); 
                                $("#Txt_Observacion_Baja").val('');
                            } 
                        };
                        Baja_Registros(xID_ACCESOS, bFECHA_BAJA, xOBS, xUSER, xSESION);
                    },
                    class: "ui-button-spl2"
                },
                {
                        ...
                        alertify.confirm(xTitulo, '<p style="color:navy;"><strong>' + xContenido + '</strong></p>',
                            function () {
                                async function Mi_Reportes() {
                                    ...
                                    xParametros.push({ ID: 'xTitulo_08', DESCRIPCION: xID_ROL });
...
function Configuracion_Inicial() {
    ...
    $("#Opt_04").click();
    $("#Opt_04_04").css({ "background-color": "#F4FA58" });
    ...
    // Tabs Nº 01: 
    $("#Cmd_Asignar").button({ icons: { primary: 'ui-icon-gear' } });
    $("#Cmd_Asignar").find(".ui-icon").css({ "background-image": "url('css/images/operaciones/new.png')", "background-position": "0" });
