...
// Variable Global
var xUbica = "", xElem = "", yMetodos = [], xSesion_Prueba = "", yUser = "", xBand_OPCIONES = 0,  xID_OPCIONES = 0, xCODIGO_OPCIONES = '', Mis_Niveles = [];
        ...
        try {
            ...
            var xMi_Modelo_01 = function () {
                ...
                self.Agregar_Enc = function () {
                    ...
                    xBand_OPCIONES = 1; xID_OPCIONES = 0;
                ...
                self.Datos_Encabezados = async () => {
                    ...
                    // Tipos de Sexos
                    let Mi_url = 'Frm_Metodos_WEB_Services.aspx/Listar_SYS_NIVELES';
                    ...
                    } else { Mis_Niveles = $.parseJSON(Mis_Registros.d.Mensaje); } 
                ...
                self.Datos_Registros = async ( yESTADO ) => {
                    ...
                    if ((yESTADO == null) || (yESTADO == undefined)) {
                        ...
                        $(".loader").fadeIn("slow");
                        let Mi_url = 'Frm_Metodos_WEB_Services.aspx/Listar_SYS_OPCIONES';
                ...
                self.selectRow_Editar = function (item) {
                    ...
                    xID_OPCIONES = parseFloat(item.ID); yMetodos[5]("Txt_Codigo_02", Mis_Niveles);

                    $('#Txt_Codigo_01').val(item.CODIGO_01); $('#Cmb_Nivel').val(parseInt(item.ID_NIVEL)); 
                    $('#Txt_Codigo_02').val(xID_OPCIONES); $('#Txt_Descripcion').val(item.DESCRIPCION); 
                    $("#Chk_Raya").prop('checked', (parseInt(item.RAYA) == 1) ? true : false);
                    $('#Txt_Orden').val(item.ORDEN); 
                ...
                self.selectRow_Reactivar = function(item){
                    ...
                    let Reactivar_Registros = async (lID, lUSER, lSESION) => {
                        let Mi_url = "Frm_Metodos_WEB_Services.aspx/Reactivar_SYS_OPCIONES";
                ...
                self.selectRow_Baja = function(item){
                    ...
                    // Se recuperan los Datos...
                    xID_OPCIONES = parseFloat(item.ID); xCODIGO_OPCIONES = item.CODIGO;
            ...
            $('#Txt_Orden').keypress(yMetodos[7]);

            // Ventanas Modales
            $("#Frm_Datos").dialog({
                autoOpen: false, width: (lastWidth * 0.65), height: 'auto', modal: true, resizable: true, show: "slide", hide: "slide",
                dialogClass: "Estilos_Ventanas_Modales",
                position: "center",
                closeOnEscape: false,
                open: function () {
                    $(this).dialog('option', 'title', (xBand_OPCIONES == 1) ? 'AGREGAR REGISTRO NUEVO' : 'EDITAR REGISTRO EXISTENTE');

                    if (xBand_OPCIONES == 1) {
                        yMetodos[5]("Txt_Codigo_02", Mis_Niveles);
                ...
                buttons: [
                {
                    ...
                    click: function () {
                        ...
                        var xCOD_01 = ($.trim($("#Txt_Codigo_01").val()).length == 0) ? "" : $.trim($("#Txt_Codigo_01").val());
                        var xORDEN = ($.trim($("#Txt_Orden").val()).length == 0) ? "" : $.trim($("#Txt_Orden").val());
                        var xCOD_02 = $("#Txt_Codigo_02 option:selected").html();
                
                        var xRAYA = ""; var checkeado = $("#Chk_Raya").is(":checked");
                        if (checkeado == true) {
                            xRAYA = "1";
                        }
                        ...
                        let xModelo_Mensajes = {
                            xDESC: 'Debe de digitar la Descripción...',
                            xCOD: 'Debe de digitar el Código Nº 01 de la Opción...',
                            xORDEN: 'Debe de digitar el Nº de ORDEN...'
                        };
                        ...
                        uEncabezado.push({ ID: xID_OPCIONES, CODIGO_01: xCOD_01, CODIGO_02: xCOD_02, DESCRIPCION: xDESC, NIVEL: ((xRAYA == "1") ? -1 : $('#Cmb_Nivel').val()), ORDEN: xORDEN, USER: xUSER });

                        // Ejecutando la Orden
                        let Guardar_Registros = async (yDatos, lSESION) => {
                            let Mi_url = "Frm_Metodos_WEB_Services.aspx/Insertar_Actualizar_SYS_OPCIONES";
                            ...
                            if (response.d.exito == false){
                                ...
                                xContenido = 'Acción Realizada con Exito...'; yMetodos[1](xContenido, "success"); 
                                xID_OPCIONES = parseFloat(response.d.Mensaje);
                                ...
                                if (xBand_OPCIONES == 1) {
                                    ...
                                    xID_OPCIONES = 0; $('.yOpt').show(); $('#xOpt').hide();
                                } 
            ...
            $("#Frm_Baja").dialog({
                ...
                closeOnEscape: false,
                open: function () {
                    $(this).dialog('option', 'title', 'BAJA AL REGISTRO Nº - ' + xCODIGO_OPCIONES);
                ...
                buttons: [
                {
                    ...
                    click: function () {
                        ...
                        let Baja_Registros = async (lID, lFECHA, lOBS, lUSER, lSESION) => {
                            let Mi_url = "Frm_Metodos_WEB_Services.aspx/Baja_SYS_OPCIONES";
                            ...
                            if (response.d.exito == false){
                                ...
                                // Limpiando Campos...
                                xID_OPCIONES = 0; xCODIGO_OPCIONES = '';

                                $("#Txt_Fecha_Baja").val(date_str); 
                                $("#Txt_Observacion_Baja").val('');
                            } 
                        };
                        Baja_Registros(xID_OPCIONES, bFECHA_BAJA, xOBS, xUSER, xSESION);
                    },
                    class: "ui-button-spl2"
                },
                {
...
function Configuracion_Inicial() {
    ...
    $("#Opt_04").click();
    $("#Opt_04_03").css({ "background-color": "#F4FA58" });
...
function Comportamiento_Objetos(){
    ...
    $('#Txt_Codigo_01').keypress(function (event) {
        if (event.which == 13) {
            $('#Txt_Codigo_02').focus().select();
            event.preventDefault(); // Detiene el proceso..
        }
    });

    $('#Cmb_Nivel').keypress(function (event) {
        if (event.which == 13) {
    ...
    $('#Cmb_Nivel').change(function (event) {
        var xValor = $('#Cmb_Nivel').val(); // Valor del ID o VALUE
        var xTexto = $("#Cmb_Nivel option:selected").html(); // Valor del Texto Seleccionado
    ...
    $("#Chk_Raya").click(function () {
        var checkeado = $("#Chk_Raya").is(":checked");
        if (checkeado == false) {
            $('.yOpt').show(); 
            if ($('#Cmb_Nivel').val() == 1) {
                $('#xOpt').hide();
            } else if ($('#Cmb_Nivel').val() == 2) {
                $('#xOpt').show();
            }
        }
        else { $('.yOpt').hide(); $('#xOpt').hide(); }
    });


    $('#Txt_Descripcion').keypress(function (event) {
        if (event.which == 13) {            
            $('#Txt_Orden').focus().select();
            event.preventDefault(); // Detiene el proceso..
        }
    });
